  ...
  _createClass(Chart, [{
    ...
    value: function componentWillReceiveProps(nextProps) {
      ...
      if (nextProps.cloned && nextProps.x != this.state.x || nextProps.cloned && nextProps.y != this.state.y) {
        var node = document.getElementsByName(this.id)[0];
        var otherNode = document.getElementsByName(1)[0];
        console.log(node.getBoundingClientRect(), 'rect from', this.id);
        this.props.resetCloneStatus(this.id);
        console.log('clone to change pos', this.id);
        this.setState({ x: nextProps.startingX });
        this.setState({ y: nextProps.startingY });
        console.log(node.getBoundingClientRect(), 'after reset', this.id);
        console.log(otherNode.getBoundingClientRect(), 'original node');
