...
var Chart = function (_Component) {
  ...
  _createClass(Chart, [{
    ...
    value: function render() {
      ...
      var cellMarginHeightAdjustment = cell.offsetHeight * .065;

      var containerRect = document.querySelector('.matrix').getBoundingClientRect();
      var newPosition = this.state.x - containerRect.x;

      console.log(this.state.x, this.state.y, containerRect.x, newPosition, cell.offsetWidth);
      return _react2.default.createElement(
        _reactRnd2.default,
        {
          size: { width: this.state.w, height: this.state.h },
          position: {
            x: newPosition + cellMarginWidthAdjustment,
            // - (cell.offsetWidth),
            y: this.state.y + cellMarginHeightAdjustment
