...
var Chart = function (_Component) {
  ...
  _createClass(Chart, [{
    ...
    value: function _copyChart(e) {
      ...
      var haveChartId = false;
      var hasRun = false;
      var attempts = 0;

      while (!haveChartId || attempts < 100) {
        console.log(attempts);
        if (!hasRun) {
          var _props$addChart = this.props.addChart(this.originCell, rows, columns, true),
              _chartId = _props$addChart.chartId;
        }
        hasRun = true;
        attempts += 1;
      }

      var focusedElement = document.getElementsByName('2');
      console.log(focusedElement.length, 'this is focused', chartId.toString());
